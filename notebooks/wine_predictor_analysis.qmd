...
Our dataset is split into two groups - 80% is split for our training data while the remaining 20% is reserved for our testing set. 
...
![Distribution of Target Class in the Data Set](../results/figures/target_distribution_plot.png){{#fig-target-dist width=40%}}

It is noted that not all possible target classes @target-dist (from 0 to 10) are present in the training data.
...
![Wine Quality Features Heatmap - Pearson Correlation](../results/figures/correlation_heatmap.png){{#fig-correlation_heatmap width=55%}}
...
![KDE Plots for all features](../results/figures/feature_distributions.png){{#fig-feature_distributions width=30%}}

The KDE Plot shows that while some of our features are normally distributed, not all of them are. There are a few features which are skewed to the right such as alcohol and sulphates. There are other features in @fig-correlation_heatmap which have multiple peaks such as volatile acidity and citric acid.

![KDE Plots for all featuresRegression Pairplot for All Features](../results/figures/feature_pairplots.png){{#fig-feature_pairplots width=15%}}
...
After performing `RandomizedSearchCV` on the `{python} model-selection-scores["test_score"].idxmax()` model, the best parameters for the model is as follows in @tbl-best_parameters: 
...
Markdown(tuning_score.to_markdown(index = False))
```
